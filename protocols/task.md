...
| 字段 | 说明 |
|:--|:--|
| `task_id` | 全局唯一 id（如 `DT-20260820-001`） |
| `project` | 所属项目/仓库 |
| `goal` | 一句话目标 |
| `objective` | 可验证的达成目标（怎么做） |
| `scope` | 范围（在/不在范围内） |
| `constraints` | 边界（只读/不 push/不改安全配置） |
| `acceptance_criteria` | 可验证成功条件（V2 级，可观测） |
| `requester_session` | 请求发起 session |
| `result_owner` | **归属 Lead / requester session**（非最终用户） |
...
- `result_owner` 必须是 Development Lead / requester session。
- 每个 task 有稳定 `task_id`，贯穿 Task → Delegation → Result → Verification 全链路。
